...
- **Know your own role vs. the panel's role.** You're interviewing as a **Senior Frontend Developer on the design system team** — not for a Design Technologist position. The panel is Design Technologists (and/or designers) — a hybrid design/code role, see Definition below. Don't try to sound like you already do their exact job; sound like someone they'd trust to build the system well and partner with closely.
...
<Callout type="info" title="Your role vs. their role — don't mix these up">
  If you're interviewing as a **Senior Frontend Developer on a design system team**, you are not applying to be a Design Technologist. The panel being Design Technologists just means the people judging you *think and evaluate through that hybrid lens* — they care a lot about collaboration, UX literacy, and systems thinking, on top of your engineering skill. You don't need to claim you already do their job. You need to show you'd be a strong, trustworthy partner for people who do.
</Callout>
...
**An engineering-only panel can't judge design fluency.** As a frontend engineer on this team, you'll work side by side with Design Technologists and designers every week — reviewing Figma files together, debating a component's API, negotiating spacing that doesn't match a token. An engineering-only panel has no way to tell if you're good at that specific kind of conversation. So the people who *do* that with you daily are put in the room to judge it directly.
...
What does a Design Technologist actually do, day to day? You're not applying for this job — but knowing what it involves tells you what your interviewers personally value and listen for, since it's the lens they naturally evaluate everyone through, including a frontend engineer joining the same team:
...
<Callout type="tip" title="Why this matters for you">
  You don't need a Design Technologist background — most Senior Frontend candidates for this round don't have one, and that's fine. But if you can describe your own past work using this same shape (prototyped something rough to answer a question, kept two sources of truth in sync, translated a technical constraint into a design-friendly explanation), it'll land well with this specific panel because it's the shape of work they respect and do themselves.
</Callout>
...
Here's the collaboration loop this round is checking that you can operate inside — and where things typically break down if you can't. Note that **you and the Design Technologist are peers on the same team**, both connected to the designer and the system, not one standing in for the other:

```mermaid
flowchart LR
  D[Designer] -->|Figma file, intent| DT[Design Technologist]
  D -->|Figma file, intent| You[You — Senior Frontend Engineer]
  DT <-->|Shared context, token sync| You
  You -->|"Here's what this costs, here's an option"| D
  You -->|Implements + documents| Sys[Design System]
  Sys -->|Tokens + components| Prod[Product Teams]
  Prod -->|Real usage feedback| You
  You -->|Feeds back constraints| D
```
...
Atlassian's own description of the Design Technologist role is worth reading before this interview, because it's a real company defining exactly the role of the people who will be interviewing you. Their framing: Design Technologists exist because "the space between design and engineering" was where good ideas were dying — either too vague for engineering to estimate, or already locked into a rigid spec before anyone tested whether the interaction actually felt good. Their Design Technologists sit inside design critiques *and* engineering planning, and their output is as often a working prototype as it is a Jira ticket.

You're not being hired to do that job yourself. But knowing it tells you what this panel values in a teammate: not "I can code what design gives me," but "I care enough to be in the room early and help shape what design gives me" — even if your own day-to-day work leans more toward the harder engineering (architecture, performance, testing) than toward prototyping.
...
<InterviewQuestion q="Tell me about a prototype or quick proof-of-concept you built to answer a design question that a static mockup couldn't answer.">
  **What the interviewer is testing:** Whether you value and are willing to use code as a design exploration tool, not just a production tool — this is the instinct the Design Technologists on the panel care about most, even for a frontend engineer who won't do this as their main job.
...
A design round with designers and Design Technologists is checking a different skill than a coding round: not "can you build it," but "can we hand you something unfinished and trust the result." You're interviewing as a Senior Frontend Developer on the design system team, not for a Design Technologist role — but the panel evaluates through that hybrid lens, so understanding what Design Technologists value (prototyping fast, keeping design and code in sync, honest two-way translation) tells you what "good" looks like to them in a teammate.
...
- You're being evaluated *by* Design Technologists as a frontend engineer, not evaluated *for* their role — don't try to sound like you already do their job, sound like a great partner for people who do.

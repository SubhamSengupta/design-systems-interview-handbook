...
- This chapter covers the React-level engineering choices behind a design system: Context, memoization, Server vs Client Components, concurrent rendering, Suspense, and hydration.
- `React.memo` does **not** stop a re-render caused by a changed Context value — memo checks props, Context subscriptions bypass that check entirely. This is the single most common interview trap in this chapter.
- Split Context by how often a value changes and who reads it (e.g. theme state vs. theme setter) — this turns "everyone re-renders" into "only the actual subscribers re-render."
- Memoizing a leaf component (Icon, Avatar, Button) pays off far more in a design system than in app code, because the same component is mounted hundreds or thousands of times across every consumer.
- Server Components rule: a Server Component can render a Client Component, but a Client Component can never directly import a Server Component — only receive one as `children`. Push `"use client"` as far down the tree as possible.
- Never put `"use client"` on a barrel/index file — it drags the entire library into every consumer's client bundle.
- `useTransition` and `useDeferredValue` reschedule work, they do not make it faster. If a computation is genuinely slow (e.g. filtering 50,000 rows), the real fix is virtualization, not a transition.
- Hydration mismatches happen when server-rendered HTML and the client's first render don't match (random IDs, `Date.now()`, `window` checks). Fix with `useId` or by deferring the non-deterministic value to `useEffect` — don't reach for `suppressHydrationWarning` as a default fix.
- Most design systems keep Context for anything internal to one compound component, and only reach for an external store (Zustand/Jotai) for cross-cutting state the system needs to observe, like a global toast queue.

## Definition

React architecture, for a design system, means the framework-level choices
that never show up in a Figma file. These choices decide whether the system
is fast, correct, and easy to use at scale. They include:

- How state moves through the component tree (Context and its alternatives).
- How re-renders are kept under control (memoization).
- Where the line between server and client code sits (Server Components).
- How low-priority updates get scheduled (concurrent rendering, `useTransition`).
- How async state shows up in the render tree (Suspense).
- How the server's HTML matches up with the client's rendered tree (hydration, streaming).

A designer reviewing one component in isolation never sees any of this. But
every choice here acts as a system-wide multiplier. Put one mistake into a
shared primitive like `Select` or `Button`, and every product team that
imports it inherits the mistake — usually without knowing why their app
feels slow.

## Problem Statement

A design system component gets imported far more often than any single
application component. A `Button` used in one team's checkout flow is also
used in the admin dashboard, the marketing site, the mobile web wrapper, and
a hundred other places nobody on the design system team has ever looked at.
This changes the cost/benefit math for every React performance decision:

- A re-render that costs 2ms inside a normal app component barely matters.
  The same 2ms wasted re-render, inside a design system primitive mounted
  500 times on one page (a data table's cells, a form's inputs), adds up to
  a full second of wasted main-thread time.
- A Context provider that re-renders its whole subtree on every state change
  is a minor annoyance in a five-component app. Inside a design system's
  compound component (`Tabs`, `Accordion`, `Menu`), the same pattern means
  every consumer's page re-renders subtrees it never asked to re-render —
  and the design system team gets blamed for a slowdown they can't even see
  in their own Storybook.
- Adding `"use client"` to a primitive that didn't need it doesn't just
  affect that one component's bundle. It forces every ancestor up to the
  nearest Server Component boundary into the client bundle too, because a
  Server Component can render a Client Component, but not the other way
  around (unless the Client Component receives the Server Component as
  `children`).
- A hydration mismatch (see below) in a component used on one page is a bug
  report. The same mismatch in a component used everywhere means React
  quietly throws away the server's HTML and re-renders it from scratch on
  every page across the whole product — a real hit to Core Web Vitals
  company-wide.

This is the core problem this chapter solves: defaults that are "good
enough" in application code are often wrong defaults in design system code,
because the fan-out multiplies both the payoff of getting them right and the
cost of getting them wrong.

## Why Does This Exist?

React is permissive by default: components re-render freely, Context
propagates to every consumer, and nothing is memoized unless you ask for it.
That's fine for application code, which has natural boundaries — a form
re-rendering when its input changes is expected and cheap. Design systems
break that assumption because they sit at the base of the component tree,
wrapping and being wrapped by everything above them. A `ThemeProvider` sits
above the entire application. A `Button` sits below almost everything else.
Both positions matter for performance, for different reasons: the provider
because it decides who re-renders, the leaf because there are so many
instances of it. React gives you no built-in protection against either
problem — you have to design for it yourself.

The second reason this is its own discipline: React itself has changed
shape under design systems over the last several years. Server Components,
concurrent rendering, and the new hydration model didn't exist when
Material-UI, Ant Design, or Chakra UI's original APIs were designed. Every
actively maintained design system has had to retrofit answers to "is this a
Server Component?" and "does this API work under React 18/19's concurrent
scheduler?" onto an API surface built before either question existed. A
design system that answers these questions up front — like newer projects
such as [Radix UI](/chapters/case-studies), Base UI, and React Aria
Components — gives every consuming team a meaningfully better default
experience than one that treats them as an afterthought.

## Historical Context

Before Hooks, class components pushed shared state (theme, locale, form
state) through prop drilling or third-party state libraries, because
`Context` (before React 16.3) was an unofficial, unstable API. React 16.3's
official `Context` API (`React.createContext`) made Context safe to use, and
design systems adopted it right away for the use case it was built for:
values that are read broadly but change rarely, like theme and locale.
Hooks (React 16.8) then made it easy to share local, compound-component
state via `useContext`, replacing the older render-prop and
higher-order-component patterns Material-UI and others had relied on — this
is why older design system code is full of `withStyles(Component)`
wrappers that newer versions replaced with hooks.

React 18 (2022) is the real turning point for this chapter. It shipped
automatic batching (fewer re-renders by default), the concurrent renderer
(which `useTransition` and `Suspense` for data both depend on), and a new
streaming SSR (server-side rendering) architecture
(`renderToPipeableStream`) that changed how hydration mismatches show up and
how much of a page can start rendering before all its data is ready. React
19 (2024) shipped Server Components as a stable, framework-integrated
feature (via Next.js's App Router and similar frameworks) — the first time
"should this design system component be a Server Component?" became a
question every component API designer has to answer, not just an
experimental one. Design systems built or heavily revised after 2023 —
Radix Themes, Base UI, Park UI (built on Panda CSS, see
[Chapter 6](/chapters/styling-architecture)) — treat RSC (React Server
Component) compatibility as a core requirement of their API design, not
something bolted on later.

## How It Works Internally

To make good decisions here, you need a working mental model of four
mechanisms: Context propagation, the render/commit cycle, the
Server/Client Component boundary, and hydration.

**Context propagation.** When a `Context.Provider`'s `value` prop changes
(by reference — Context does not deep-compare it), React re-renders every
component that calls `useContext` on that context, *whether or not that
component's own props or state changed*. Crucially, this re-render skips
right past `React.memo` on the *consumer* — memoizing a component does not
stop it from re-rendering when a context it reads changes, because
`useContext` subscribes independently of the props/state check `memo`
...
re-renders on every keystroke in a searchable menu — when the real cause is
that it reads `useContext(MenuContext)`, and the provider hands it a new
object literal as `value` on every parent render.

**Render vs. commit.** React's render phase (calling your function
components to build a new element tree) is not the same as the commit phase
(applying the resulting diff to the DOM). Memoization tools (`React.memo`,
...
commit phase. This matters because the "waste" from an unnecessary render
usually isn't DOM work at all — React's diffing means an unchanged subtree
commits nothing to the DOM. The real cost is the JavaScript work: re-running
component functions, re-running hooks, re-diffing children. For a design
system with hundreds of live instances on a page, that JavaScript cost is
where the wasted time actually goes.

**The Server/Client boundary.** Under RSC, the module graph is split at
build time by the `"use client"` directive. Any file with that directive at
the top — plus everything it imports — gets bundled for the browser and
hydrated on the client. A file without the directive is a Server Component:
it runs once on the server (or at build time, for static routes), produces
a serialized description of its output, and ships *zero* JavaScript to the
client for itself. The critical rule: a Server Component can import and
render a Client Component, but a Client Component cannot directly import a
Server Component — it can only receive one as `children` or another prop
passed down from a Server Component ancestor. This is why the client
boundary should sit as low in the tree as possible: every Client Component
you declare pulls its entire import subtree into the client bundle. Adding
`"use client"` to a component just because it needs one `onClick` handler
drags its whole visual subtree — icons, text-formatting helpers, layout
children — into that bundle too, unless you deliberately restructure that
subtree to be passed in as `children` from a Server Component parent.

**Hydration.** On the client, React walks the server-rendered DOM and the
client's own render output *in lockstep*, trying to reuse existing DOM nodes
and attach event listeners instead of re-creating markup from scratch. It
compares the first client render's output, node by node, against what the
server sent down as the initial HTML. If they diverge — different text,
different attributes, a different element type — React logs a hydration
mismatch, throws away the mismatched subtree (or, in some cases, the whole
tree), and re-renders it from scratch on the client. That wastes whatever
performance benefit server-side rendering was supposed to provide for that
subtree.

## Architecture

The diagram below shows how these four mechanisms fit together on a typical
...
Read this diagram top to bottom: the page itself and most static content
stay Server Components (zero client JavaScript). The client boundary is
pushed down to exactly the components that need interactivity or state
(`ThemeProvider`, `Combobox`). The compound component's internal state lives
in its own narrow context instead of leaking into the page's state. An
async leaf is wrapped in `Suspense` so its fallback can stream in without
blocking the rest of the page. This is the shape every other section in
this chapter is arguing toward.
...
chapter. Each compound component ships its own narrowly-scoped context
(`createSelectContext`, `createDropdownMenuContext`) instead of one shared
"design system context." That means a `Select`'s open/close state changing
never re-renders an unrelated `DropdownMenu` on the same page, and each
context's value only recomputes when its specific slice of state changes.
React Aria Components (Adobe's headless library, and the pattern Adobe's
own [Spectrum](/chapters/case-studies) is migrating toward) goes further:
it offers a *render props* API (`children` as a function that receives
state) specifically to avoid forcing consumers into a Context subscription
at all for simple cases. You get the current state as a plain function
argument instead of subscribing to a provider — which avoids Context
re-render fan-out entirely for components that don't need persistent shared
state.

On the Server Components side, shadcn/ui's "copy-paste, not npm-install"
distribution model was driven partly by this exact problem. Because
consumers own the source code in their own repo, they can freely decide
which parts of a component are Server vs. Client Components for their own
app, instead of inheriting one library author's one-size-fits-all `"use
client"` boundary. Vercel's own component examples and the Next.js docs
consistently model this by pushing `"use client"` down to a small
`<Interactive>` wrapper around, say, a `<Tooltip>`'s trigger, while the
tooltip's static content stays server-rendered.

## Implementation Example

### Context, scoped correctly

A theme provider is the textbook "yes, use Context" case: the value is read
broadly, changes rarely, and prop-drilling it through every component would
be absurd.
...
`Tabs` is the textbook compound-component case: `Tabs.List`, `Tabs.Trigger`,
and `Tabs.Content` all need to agree on which tab is active, without the
...
This is the same compound-component shape covered in
[Chapter 3](/chapters/component-architecture). The point here is narrower:
the context is *private* to this file, scoped to one `Root` instance, and
carries only what `Trigger`/`Content` actually need — not the whole design
system's state.
...
<Callout type="tip" title="suppressHydrationWarning is a scalpel, not a bandage">
  `suppressHydrationWarning` only silences the warning for the one DOM node
  it's applied to, one level deep. It does not stop React from discarding
  and re-rendering mismatched children, and it does nothing for structural
  mismatches (a different tag, a different number of children). Legitimate
  uses are narrow: a `<time>` element whose text is expected to differ
  (timestamp formatting, as shown above), where you've decided the visible
  flash is an acceptable tradeoff. Using it to silence a mismatch caused by
  a `window` check or `Math.random()` just hides the symptom — fix the real
  source of non-determinism instead.
</Callout>

## Advantages

Getting this architecture right buys a design system several wins that
compound over time. Splitting Context turns an O(n) re-render problem
(every consumer of one monolithic context re-renders on any change) into an
O(1) one (only the actual subscribers of the changed slice re-render) —
and because design system context providers sit above entire applications,
this difference is often the whole performance story for a page.
Correctly-scoped memoization on leaf primitives means the design system
team pays the cost of getting memoization right *once*, centrally — instead
of every consuming team having to remember to wrap their own usage in
`memo`. Pushing the Server/Client boundary down means consumers on
RSC-capable frameworks get a smaller client bundle for free. In real
migrations, moving mostly-presentational primitives (`Card`, `Badge`,
`Avatar` markup, `Divider`) out of the client bundle has shaved tens to low
hundreds of KB off first-load JavaScript for consumers who used to import
the whole library as client code. `Suspense` and `useTransition` let async
and expensive UI degrade gracefully — a slow avatar image or a 10,000-option
filter no longer blocks the rest of the page or the keystroke that
triggered it. And hydration-safe patterns (`useId`, deferred relative-time
rendering) mean consuming teams stop filing "flash of wrong content" and
"hydration mismatch warning spam" bugs against components they didn't write
and can't easily debug themselves.

## Disadvantages

None of this comes for free. Splitting one context into several means more
provider nesting, more files, and a steeper learning curve for engineers new
to the codebase, who now have to learn *which* context holds *which* slice
of state instead of one obvious `DesignSystemContext`. Aggressive
memoization has a real maintenance cost too: `useCallback`/`useMemo`
dependency arrays are a common source of stale-closure bugs, and every one
should be justified by profiling — memoizing something that already renders
cheaply just adds comparison overhead for no benefit. The Server/Client
boundary decision isn't a one-time cost either. Every new feature added to
a "Server Component-safe" primitive has to be re-checked for whether it
introduces a client-only dependency (an event handler, a browser API, a
stateful hook), and getting this wrong produces a build-time error that
confuses engineers who don't yet have a mental model of the RSC module
graph. `useTransition` and `Suspense` both require the whole team to
understand React's concurrent rendering model well enough to reason about
when a component's render can be interrupted mid-way — a component with a
side effect that assumes it always runs to completion (a non-idempotent
effect, a mutation inside render) can behave subtly wrong under concurrent
rendering in ways that only show up under real user interaction, not a
quick manual test.

## Alternatives

The recurring decision this chapter is really about is: *how should state
be shared across a component tree, and where should the client/server line
be drawn?* For sharing state specifically, Context is React's built-in
default, but it isn't the only option a design system team reaches for.
...
React Aria, Ariakit, Chakra, and MUI's unstyled base all do it. They reach
for an external store only for cross-cutting, app-shaped state that the
design system needs to *observe* rather than own. A global toast or
notification queue is the most common example: toasts get triggered from
anywhere in the app, not from within a single compound component's
subtree, which is exactly the shape Context handles badly — every
toast-triggering component would need to be a descendant of the toast
provider, which isn't true in a real app. Large companies with framework
flexibility requirements (Shopify's Polaris, Atlassian's Design System)
lean toward keeping the dependency graph minimal, using plain Context plus
`useReducer` for anything more complex than a single piece of state. The
reason: an external store dependency becomes a version-compatibility burden
multiplied across every consuming team the moment it's baked into the
library's own `package.json`, rather than left as a choice the consumer
makes for their own app-level state.

## Tradeoffs

Every decision in this chapter trades authoring simplicity against
consumption-time performance — and the right answer here leans harder
toward consumption-time performance than it would in application code,
because of the fan-out argument from the Problem Statement. Splitting
...
again. Pushing `"use client"` down costs the team a design review of "does
this prop introduce a client dependency" on every PR, and buys every
RSC-capable consumer a smaller bundle without them doing anything. The one
place this logic flips is `useTransition`/`Suspense`: these add real
conceptual overhead (interruptible rendering, idempotency requirements) for
components that aren't actually expensive to render. A five-item `<Select>`
gains nothing from `useTransition` — adopting it anyway just adds
complexity consumers have to reason about for no payoff. Reserve
concurrent-rendering APIs for the specific components where the fan-out or
data size is large enough to matter (large `Combobox`/`Select` option
lists, data-heavy `Table` filtering), not as a blanket default.

## Performance Considerations

The measurable levers, roughly ordered by typical impact for a mature
design system:

1. **Context granularity.** Profile with React DevTools' Profiler tab, sort
   by "why did this render," and look specifically for components
   re-rendering with an unchanged prop diff — that's the signature of a
   context-driven re-render. Splitting a monolithic `DesignSystemContext`
   into `ThemeContext` + `LocaleContext` + `DensityContext` is usually the
   single highest-leverage fix available, because it's a small, one-time
   effort against a payoff that scales with every consumer.
2. **Leaf memoization with measured payoff.** Memoize components that are
   (a) rendered many times per page and (b) receive fairly stable props
   most of the time. Icon, Avatar, Badge, Chip, and table-cell-shaped
   primitives are the textbook cases. Don't memoize components rendered
   once per page (a page header, a modal) — there's no fan-out to earn back
   the comparison cost.
3. **Bundle size via the Server/Client split.** Run a bundle analyzer
   ...
   primitives out of `"use client"` files. This is the performance win
   consumers notice most directly in Lighthouse/Core Web Vitals scores,
   because it directly cuts Total Blocking Time and Time to Interactive.
4. **Deferred/transitioned updates for expensive derived state.**
   ...
   they keep the *input handling* responsive while expensive computation
   happens at lower priority. Reach for these only once profiling shows an
   interaction (typing, filtering) is actually janky, not preemptively.
5. **Hydration cost.** Every hydration mismatch means React throws away and
   re-renders a subtree. Measure this with the "Recoverable Errors"
   reporting React 18+ exposes, not just by eyeballing the console —
...
virtualization discussion this connects to.
...
specifically *scale* problems. They show up once a design system has
enough consumers that: (a) a single page uses dozens of instances of the
same primitive, (b) enough different teams are building on the compound
components that nobody remembers the original context shape's intent
anymore, and (c) the org runs both RSC and non-RSC consumers at the same
time (a Next.js App Router app and a Vite SPA both importing the same
package) — which is an increasingly normal state for a company mid-way
through a migration. Design systems built to serve this kind of mixed,
multi-framework consumer base generally version their client-boundary
decisions carefully. Removing a `"use client"` directive in a minor version
is technically a breaking change for consumers relying on tree-shaking it
differently. They also treat context-shape changes like any other public
API change, because compound-component context, even though it's
"internal," is observable through behavior — whether nested instances
interfere with each other, whether SSR output matches.
...
<Callout type="danger" title="Treating useTransition as a performance fix for slow computation">
  `useTransition` reschedules *when* a render happens — it doesn't make the
  render itself faster. If filtering 50,000 items genuinely takes 400ms,
  wrapping it in a transition keeps the input responsive, but the list
  still takes 400ms to update. The real fix is virtualization (see
...
- Memoize the context `value` itself with `useMemo`, and memoize consumer
  leaf components with `React.memo` — treat these as two separate fixes
  that are both necessary. Doing only one solves half the problem.
...
- Gate any render-time environment check (`typeof window !== "undefined"`)
  behind `useEffect` plus local state, never inline in the render body.
...
- Document the client/server boundary decision in the component's own
  README or Storybook doc page, so consumers on RSC frameworks know
  without reading the source whether a component is safe to import into a
  Server Component tree directly.

This connects to the API design principles in
...
onto every consumer stays easier to keep server-renderable as it grows.
...
<InterviewQuestion q="A team reports that their page re-renders every design-system Button whenever an unrelated modal opens. Where do you look first?">
  **What the interviewer is testing:** Whether you have the right mental
  model of Context propagation versus `React.memo`, and can trace a
  symptom back to its root cause instead of guessing at fixes.

  **Poor answer:** "Wrap Button in `React.memo`."

  **Good answer:** Checks whether Button reads a shared context, and
  whether that context's provider re-renders (and hands down a new
  `value` reference) when the modal's open state changes.
  ...
  **Excellent senior answer:** Explains that `memo` doesn't block
  ...
  `style` object prop) and describes how they'd check which.

  **Common mistakes:** Reaching for `memo` as a reflex fix without
  diagnosing whether the cause is Context, an unstable prop reference, or
  an actual state change; not knowing that `memo` and Context are
...
<InterviewQuestion q="Why might you choose useCallback on a design-system Button's internal click handler even if profiling shows Button itself renders fast?">
  ...
  **Excellent senior answer:** Notes the real leverage argument — Button
  ...
  reference that lets a memoized Icon skip a re-render is a small win
  multiplied by every Button on every page in every consuming app. That's
  categorically different math than the same optimization in a one-off app
  component.

  **Common mistakes:** Applying `useCallback` everywhere as a reflex
  without being able to say what specifically it prevents downstream; not
  recognizing that `useCallback` with an empty or wrong dependency array
  can introduce stale-closure bugs, which is the real cost side of this
  tradeoff.
</InterviewQuestion>
...
<InterviewQuestion q="Explain a hydration mismatch: what is it mechanically, and give a concrete design-system example along with the fix.">
  ...
  **Good answer:** Explains React compares server HTML to the first client
  ...
  **Excellent senior answer:** Gives the `useId` fix for generated ids
  specifically (not just "use useEffect" — `useId` solves it without a
  client-only re-render at all), explains the relative-time pattern
  (render a deterministic value first, swap after mount), and states
  precisely what `suppressHydrationWarning` does and doesn't cover — it
  silences the warning one node deep and doesn't fix structural
  mismatches.
...
<ManagerQuestion q="An engineer on your team wants to add a third-party state management library as a dependency of the design system, to simplify some cross-component state. How do you evaluate that request?">
  **What the interviewer is testing:** Judgment about dependency ownership
  and the uneven cost of adding dependencies to a widely-consumed package
  ...
  **Excellent senior answer:** Frames it explicitly: every dependency
  added to the design system's `package.json` becomes every consuming
  team's transitive dependency too, with a version this team now has to
  keep compatible across the whole consumer base — a cost application
...
rendering — is tuned for application code, where mistakes stay locally
contained. A design system flips that assumption. Because every primitive
...
non-deterministic values) instead of reaching for warning-suppression as a
first resort. None of these are exotic React features — every tool
discussed here (`memo`, `useMemo`, `useCallback`, `useTransition`,
`Suspense`, `useId`) is documented, mainstream API. What this chapter is
really teaching is how to recognize *which* of these problems you have from
a symptom, and how to apply the fix at the layer (Context shape, boundary

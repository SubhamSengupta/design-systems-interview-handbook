...
Accessibility, in the design system context, is the practice of building
components whose behavior — not just appearance — works correctly for
people using a keyboard instead of a mouse, a screen reader instead of a
screen, or an operating-system setting like reduced motion or high contrast
instead of default rendering. It is measured against WCAG (Web Content
Accessibility Guidelines), implemented through semantic HTML and ARIA, and,
critically for this chapter, it is an *architectural* property of a
component, not a checklist applied after the component ships. A `Dialog`
that traps focus, restores it on close, and is announced correctly by a
screen reader is not a "more accessible version" of a dialog — a dialog
that doesn't do those things is broken for a defined, sizeable population of
real users, in the same way a dialog that throws a JavaScript error for some
users would be considered broken.

## Problem Statement

Accessibility bugs are two problems that compound each other. First, they're
invisible to the majority of engineers testing a feature the normal way —
click through it with a mouse, look at it, ship it. A missing `aria-label`,
a focus trap that doesn't restore focus on close, a custom dropdown that
can't be operated with arrow keys: none of these show up in a visual QA
pass. Second, when they do get caught, they're almost always caught per
feature, per team, after the component has already shipped — which means
the same class of bug (an unlabeled icon button, a modal that doesn't trap
focus) gets independently reimplemented and independently broken by every
team building their own version of that pattern.

<Callout type="tip" title="The core leverage argument">
  This is the single strongest argument for centralizing accessibility work
  inside a design system rather than treating it as a per-feature
  responsibility: **fix focus-trapping once, correctly, inside `Dialog`, and
  every consumer of `Dialog` — today and in every future feature built on
  it — inherits the fix for free.** The alternative is teaching every
  feature team the WAI-ARIA Authoring Practices for dialogs, trusting every
  team to implement it correctly, and re-auditing every team's
  implementation independently forever. A design system turns an
  organization-wide, unbounded accessibility liability into a single,
  auditable, one-time engineering investment. This is why accessibility
  work is consistently one of the highest-ROI investments a design system
  team can make, and why it should be treated as core system architecture,
  not a "nice to have" backlog item.
</Callout>

The problem this chapter solves is narrower than "how do I make a webpage
accessible" — it's "what specifically does a design system need to get
right, once, at the primitive layer, so that every consuming team's product
is accessible by construction rather than by diligence."

## Why Does This Exist?

Accessibility as a discipline exists because the web's native elements
(`<button>`, `<select>`, `<a>`) come with keyboard operability, focus
management, and screen reader semantics built in, for free, by the browser
— and the moment a team reaches for a custom-styled `<div>` with an
`onClick` handler instead, all of that free behavior disappears and has to
be manually rebuilt. This happens constantly in design systems specifically
because visual design requirements (a dropdown that looks nothing like a
native `<select>`, a toggle that looks nothing like a checkbox) push teams
away from native elements toward custom markup, and custom markup is where
all the free browser behavior has to be hand-reimplemented or is silently
lost. Design systems exist partly to solve exactly this: give teams a
`<Select>` that *looks* like the Figma spec but *behaves* like the native
element underneath, so nobody downstream has to choose between visual
fidelity and accessibility.

Legal and regulatory pressure is the second real driver. The ADA (in the
US), the European Accessibility Act, and similar regulations have produced
real litigation exposure for inaccessible commercial products — this is not
a hypothetical risk for large consumer or enterprise-software companies, and
it's a large part of why accessibility budget gets approved at the
executive level in a way that other quality investments sometimes don't.

## Historical Context

Section 508 (US federal accessibility law, 1998, significantly updated
2017) and WCAG 1.0 (1999) predate modern component-driven frontend
development entirely — they were written against static HTML documents.
WCAG 2.0 (2008) and 2.1 (2018) extended the guidelines to cover dynamic,
scripted interfaces, which is when ARIA (first published as a W3C spec in
2008, stabilized through the 2010s) became necessary: ARIA exists
specifically to describe the semantics of the custom, JavaScript-driven
widgets that plain HTML has no vocabulary for (a combobox, a tree, a tab
panel). The WAI-ARIA Authoring Practices Guide (APG) — the canonical
reference for "how do I build an accessible `[Dialog/Menu/Tabs/...]`" — has
been iteratively refined since and is the primary source every serious
design system's accessible-pattern implementation is checked against.

The design-systems-specific history matters too: early large design
systems (Bootstrap's original dropdown/modal JS, jQuery UI) had
...
from day one, with the APG patterns as the literal specification for each
component's behavior, not a retrofit. This is why the current default
recommendation for a new design system's interactive primitives is almost
always "build on one of these, don't hand-roll focus trapping and roving
tabindex from scratch" — see the [Alternatives](#alternatives) section
below.

## How It Works Internally

Four mechanisms do almost all of the work described in this chapter's
topic list: the accessibility tree, ARIA's role/state/property model, focus
management, and assistive technology's consumption of live regions.

**The accessibility tree.** The browser builds a parallel tree alongside the
DOM — the accessibility tree — by computing, for every element, an
accessible **role** (what kind of thing is it — button, checkbox, dialog),
an accessible **name** (what should it be announced as — from text content,
`aria-label`, `aria-labelledby`, or associated `<label>`), and an accessible
**state** (checked, expanded, disabled, selected). Screen readers query
*this* tree, not the visual DOM. Every ARIA attribute you add is either
directly setting a slot in this tree (`role`, `aria-label`) or modifying how
the browser computes one (`aria-hidden` removes a subtree from the tree
entirely). This is why two visually identical elements — a real `<button>`
and a `<div onClick>` — produce a completely different accessibility tree
node unless you manually give the `div` `role="button"`, `tabIndex={0}`, and
keyboard handlers to match.

**ARIA's first rule.** The WAI-ARIA spec itself states this explicitly: *if
a native HTML element or attribute has the semantics and behavior you
require, use it instead of re-purposing an element and adding ARIA.* ARIA
can describe a role, but it cannot grant behavior — adding `role="button"`
to a `div` tells assistive technology "announce this as a button," but the
browser does not thereby make it focusable, or make Enter/Space activate
it, or add it to Tab order. Those all have to be manually added back. This
is why `role="button"` on a `div` is a code smell in any component review —
it's almost always cheaper and more correct to render an actual `<button>`
and override its default styles than to reconstruct a button's behavior
from ARIA plus manual event handlers.

**Focus management.** The browser tracks exactly one focused element at a
time (`document.activeElement`), and only elements that are focusable
(native interactive elements, or anything with an explicit `tabindex`) can
receive it. Two focus mechanisms matter for composite widgets: **Tab order**
moves focus between *independent* interactive elements (moving from a
dialog to the field after it), and **roving tabindex** (or `aria-activedescendant`)
...
ever encountered and one that traps them or skips content.
...
they do not automatically announce content that changes *without* a focus
change — a toast notification appearing, a form validation error appearing
...
content) tells the browser to notify the AT whenever that container's
content changes, even without a focus move, and the AT queues an
...
native element that clears it automatically, and only build the ARIA +
keyboard + focus-management stack by hand when no native element covers the
required interaction — at which point every layer (role/state, keyboard
model, focus management) has to be deliberately engineered to feed a
correct accessibility tree.

## Real-World Example

Radix UI's `Dialog` primitive is the most-cited reference implementation in
the industry for this chapter's Dialog pattern: it composes
`focus-trap`-equivalent logic internally, restores focus to the triggering
element on close, sets `aria-modal="true"` and wires `aria-labelledby`/
`aria-describedby` automatically from its `Dialog.Title`/`Dialog.Description`
sub-components, and closes on Escape — all without the consumer writing any
of that logic themselves, only supplying markup and styles. GitHub's
Primer design system and Shopify's Polaris both publish explicit
accessibility documentation per component (keyboard interaction tables,
ARIA usage notes) as a first-class part of their component docs, not an
appendix — Polaris in particular documents exact keyboard shortcuts per
composite widget in the same reference table as props. Adobe's Spectrum
(via React Aria, referenced further in
[Chapter 3](/chapters/component-architecture) and
[Chapter 14](/chapters/case-studies)) is built by an accessibility-focused
team from the ground up and is frequently the reference other design
systems' engineers consult when implementing a pattern the APG covers only
loosely, such as a combobox with async-loaded options.
...
<Callout type="tip" title="Don't hand-roll this in application code">
  The Dialog above is deliberately shown in full because understanding the
...
[Chapter 2](/chapters/design-tokens): motion, like color and spacing, should
be a token category, not a hardcoded value scattered across component
files, precisely so it can be gated globally like this.

### Color contrast — the actual math

WCAG 2.1's contrast requirement is a ratio between the relative luminance of
foreground and background colors, computed from each color's sRGB channels
via a gamma-correction formula, then expressed as `(L1 + 0.05) / (L2 + 0.05)`
...
- **7:1 / 4.5:1** — the AAA equivalents of the above, rarely mandated for
...
[Chapter 2](/chapters/design-tokens)) can't be designed purely on visual
taste — every semantic pairing a design system ships as a default (`text-
primary` on `surface-default`, `text-on-brand` on `brand-500`) has to be
checked against this formula and locked in as a *contract*, not a
suggestion, because a designer adjusting a brand color's lightness by a few
percentage points in Figma can silently drop a previously-compliant pairing
below 4.5:1.

## Advantages

Centralizing accessibility in the design system converts an unbounded,
recurring liability (every team re-implementing and re-breaking the same
patterns) into a bounded, one-time investment with compounding returns —
every new consumer of `Dialog`, `Menu`, or `Tabs` inherits a correct
implementation without needing an accessibility specialist on their own
team. It also produces consistent behavior across the entire product
surface, which matters specifically for AT users: a screen reader user who
learns how one product's dialogs behave can rely on that behavior
everywhere the same design system is used, the same way sighted users rely
on visual consistency. And centralization makes accessibility auditable —
one axe-core/manual-testing pass against the design system's Storybook
covers the primitive behavior for every consumer, rather than requiring a
full audit of every individual product surface.
...
added/removed at runtime) can be worse than not attempting the pattern at
all, because it produces a false sense of coverage. Centralizing also means
a bug here has outsized blast radius in the other direction from the
Chapter 4 argument — an accessibility regression in `Dialog` breaks every
consumer's modals simultaneously, which is exactly the reasoning that
justifies the investment but also raises the bar for how carefully changes
to these primitives need to be tested and reviewed. And design-system-level
accessibility work can't fully absorb feature-level responsibility: a
correctly built `Dialog` primitive doesn't guarantee a feature team writes a
meaningful `aria-label`, structures heading levels correctly, or writes
accessible copy — the primitive removes an entire class of bug, not all of
them.
...
Dialog, Menu, Tabs, Tooltip, Combobox are all well-covered by the WAI-ARIA
APG and by mature libraries, and the edge cases (focus restoration to a
removed trigger, `aria-activedescendant` vs roving tabindex tradeoffs,
right-to-left keyboard direction) have already been debugged by teams with
far more AT-testing resources than most product companies can justify
standing up internally. This is why the current generation of design
systems — shadcn/ui, Vercel's own component patterns, most new internal
enterprise systems built since ~2021 — build their styled components on top
of Radix UI or React Aria rather than reimplementing ARIA patterns, and why
even design systems with a long pre-existing custom implementation (older
versions of Material-UI, Ant Design) have incrementally migrated toward
these unstyled primitives or React Aria's hooks under the hood over time.
The build-vs-adopt calculus only flips toward "build" when a company the
size of Adobe, Google, or Microsoft can justify a dedicated internal
accessibility engineering team whose full-time job is exactly this — most
companies are correctly better served putting that same engineering time
into design-system-specific integration and design-token-driven contrast
enforcement instead of re-deriving what Radix or React Aria already ship.
...
faster," it's "ship a liability every consuming team inherits silently
...
real tradeoff in engineering time: automated tools are fast and catch a
...
save time produces a false sense of coverage that's arguably worse than no
testing at all, because it creates unwarranted confidence.

## Performance Considerations

Accessibility and performance intersect in a few concrete ways relevant to
a design system specifically. Live regions (`aria-live`) should be
sparingly and deliberately placed — a page with many overlapping live
regions, or a live region whose content updates at high frequency (a
progress percentage updating every 100ms), produces a genuinely bad,
overwhelming experience for AT users, distinct from any visual performance
metric but just as real a regression. Focus-trap implementations that
re-query the DOM for focusable elements on every keystroke (rather than
caching the list and invalidating it only on relevant DOM mutations) can
...
the accessibility tree on DOM mutation, so components that re-render large
subtrees unnecessarily (see [Chapter 4](/chapters/react-architecture) on
context-driven re-render fan-out) impose a real, if less commonly measured,
cost on AT users' navigation smoothness, on top of the visual jank sighted
users experience from the same root cause.

## Scalability Considerations

At small scale, one team can hold the entire set of accessibility patterns
in their head and manually verify each feature. That stops working the
moment a design system has enough consumers that most product engineers
building on top of it have never read the WAI-ARIA APG and shouldn't need
to — which is the normal state for any design system past its first year.
Scaling accessibility means investing in things that catch regressions
*without* requiring every consuming engineer to be an accessibility expert:
...
accessibility regressions with the same severity classification as
functional bugs or visual regressions in the design system's own release
process (see [Chapter 11](/chapters/governance)) — an accessibility
regression that ships in a minor version and silently breaks focus
management for every `Dialog` consumer is not a lower-severity bug than a
visual one, even though it's frequently treated that way in practice.
...
<InterviewQuestion q="Why should accessibility work live in the design system rather than be handled per-feature by each product team?">
  ...
  **Excellent senior answer:** Frames it explicitly as leverage: an
  ...
  reimplemented and independently broken by every team, forever, with no
  way to audit it centrally. Adds that this doesn't remove all
  feature-level responsibility — copy, heading structure, and content-level
  a11y still belong to feature teams.
...
Accessibility in a design system is not a per-component checklist run at
the end — it's an architectural property that has to be designed in from
the start, because the leverage argument cuts both ways: a correct `Dialog`
implementation benefits every consumer forever, and a broken one breaks
every consumer's modals simultaneously. The practical foundation is WCAG's
POUR principles enforced at AA, native HTML preferred wherever it suffices,
and ARIA reserved for genuinely novel widgets — with the discipline to
remember ARIA only adds semantics, never behavior, so keyboard interaction
and focus management always have to be engineered by hand alongside it.
Composite widgets need roving tabindex or `aria-activedescendant` for
intra-widget navigation, dialogs need full focus trapping and restoration,
async updates need deliberately-placed live regions, color pairings need to
be checked against the actual luminance-based contrast formula rather than
eyeballed, and motion needs to be gated behind `prefers-reduced-motion` at
the token layer. Automated tooling (axe-core, the Storybook a11y addon)
should be a mandatory CI floor, never mistaken for complete coverage —
manual keyboard and real screen reader testing remain non-negotiable
because roughly 60-70% of real-world accessibility failures are
structurally invisible to automated scanners. Most new design systems today
correctly build these patterns on top of Radix UI or React Aria rather than
reimplementing the WAI-ARIA Authoring Practices from scratch, reserving
custom implementation for genuinely novel interactions.
